...
const ProviderSettings: React.FC = () => {
  ...
  return (
    <>
      <ProviderButton onClick={() => setIsModalOpen(true)}>
        <ProviderButtonContent>
          <ProviderButtonText>
            <span>{currentModelName}</span>
          </ProviderButtonText>
          <FiChevronDown size={12} style={{ opacity: 0.6 }} />
        </ProviderButtonContent>
      </ProviderButton>
      ...
      <Modal visible={isModalOpen} onRequestClose={() => setIsModalOpen(false)}>
        <div style={{ padding: '20px' }}>
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            >
              {getAvailableModelsForProvider(tempSettings.provider).map(
                model => (
          ...
          >
            <TestButton onClick={handleTestConnection}>
              Testar Conexão
            </TestButton>
            <SaveButton onClick={handleSave}>Salvar Configurações</SaveButton>
          </div>
        </div>
      </Modal>
    </>
